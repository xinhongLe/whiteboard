 ...
 * @returns 
 */
export const getTouchesCenter = (touchList: TouchList,canvasConfig:ICanvasConfig) => {
    const pointOne = touchList[0];
    const pointTwo = touchList[1];
    return { x: (pointOne.pageX + pointTwo.pageX) / 2 - canvasConfig.scrollX, y: (pointOne.pageY + pointTwo.pageY) / 2 + canvasConfig.scrollY };
};
